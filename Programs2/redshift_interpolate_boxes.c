...
/*****************************************************************************************
   ...
   forthe velocity field, the filename list (argv[2]) should just list one velocity component
...
int main(int argc, char ** argv){
  ...
  */
  ...
  // allocate memory forthe three boxes
  ...
  while(!feof(BOX_LIST)){
    ...
    while(z < z2){ // until we move to the next set of boxes
      ...
      } // if(slice_ct == HII_DIM), we are now continuing with a new interpolation box
      ...
      slice_ct++;
      z -= dR / drdz(z);
    } // while(z < z2), done with this pair of boxes, moving on to the next redshift pair
