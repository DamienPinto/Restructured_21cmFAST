...
double FgtrM_st_PL(double z, double Mmin, double MFeedback, double alpha_pl){
  ...
  gsl_function F;
  double rel_tol  = 0.001; //<- relative tolerance
  gsl_integration_workspace * w_lower = gsl_integration_workspace_alloc (1000);
  gsl_integration_workspace * w_upper = gsl_integration_workspace_alloc (1000);
...
double dFdlnM_st_SFR(double lnM, void *params){
  ...
  double Alpha_esc = vals.pl_esc;
    ...
    (Fstar*Fstar10 < FRACT_FLOAT_ERR) Fstar = FRACT_FLOAT_ERR;
  }

	if(Fesc*Fesc10 > 1.){
    Fesc = 1./Fesc10;
  }
	else if(Fesc*Fstar10 < FRACT_FLOAT_ERR){
    Fesc = FRACT_FLOAT_ERR;
  }
