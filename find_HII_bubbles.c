...
int main(int argc, char ** argv){
  ...
  *error_message = '\0';
  ...
  } // if((mean_f_coll_st*ION_EFF_FACTOR < HII_ROUND_ERR))
  ...
  if(USE_TS_IN_21CM){
    ...
    fclose(F);
    F = NULL;
  } //end: including reionization from xrays?
  ...
  while(!LAST_FILTER_STEP && (M_MIN < RtoM(R)) ){
      ...
      for(y=0; y<HII_DIM; y++){
        for(z=0; z<HII_DIM; z++){
          if(USE_HALO_FIELD){
            ...
            else{ // the entire cell belongs to a collpased halo...  this is rare...
              Splined_Fcoll =  1.0;
            }
          }
    ...
    R /= DELTA_R_HII_FACTOR;
  } // while(!LAST_FILTER_STEP && (M_MIN < RtoM(R)) ), END OF LOOP THROUGH FILTER RADII
